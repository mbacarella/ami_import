...
let raise_transport_error ~name err =
  failwithf "%s: %s" name (Awsm_http.Io.Error.sexp_of_call err |> Sexp.to_string_hum) ()
;;
...
let import_image ~cfg ~name ~architecture ~image =
  let region =
    Option.value_exn ~message:"AWS config does not have region set" cfg.Cfg.region
  in
